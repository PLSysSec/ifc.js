...
mutable references \Red{and errors}, prepared for combination with an
...
\inferrule[T-ass]
{ }
{| conf tS (ta := tv) -> conf (tS [ ta mapsto tv ]) tv |}
\end{mathpar}
...
labels (|canflowto|, |lub| and |glb|),
    \item Operations for inspecting (\textbf{getLabel}) and modifying (\textbf{setLabel}) the current label of the thread (note a thread can only increase its label, and not decrease it),
