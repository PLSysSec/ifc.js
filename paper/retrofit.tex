...
flow control language}}. While our technique will be indifferent to the
actual target language, for concreteness we give a simple, untyped lambda calculus to illustrate some examples, as well as express
how the language should be prepared for interoperability
in the style of Matthews and
Findler~\cite{Matthews:2007:OSM:1190216.1190220}.
We then equip this target language with primitives for implementing dynamic
...
mutable references, prepared for combination with an
...
with the partial order |canFlowTo|.
...
evaluation context |curlyBlueE| represents the context
of the current task which is running, which has three important pieces
of state: the global message queues |iS|,
the current label |il| (which protects the program counter of the task)
and the task ID |id|.
...
We consider asynchronous communication primitives that allow different
tasks to exchange messages.
When a task is sending a message using |send|, it also labels that
...
The particular choice of communication primitives has been motivated by
inter-worker communication in web browser APIs~\Red{cite}, but
other choices such as blocking communication or labeled channels
would also be possible.
We note, however, that blocking receive can be implemented using
our asynchronous primitives as follows:
\begin{code}
  brecvf ix1 ix2 ie @=
  tab tab tab (IT (let tx = TI (recvf ix1 ix2 ie (IT tx)) in tx))
\end{code}
...
\begin{itemize}
    ...
    \item We reinterpret |CE| to be evaluation contexts on task pools, providing definitions for |curlyRedE| and |curlyBlueE|.  These rules only operate on the first task in the task pool, which by convention is the only task executing.
...
We note that it is possible to add rules for a natuaral embedding of
