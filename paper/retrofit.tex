...
ie   ::=  iv | x | y | ie lop ie | getLabel | setLabel ie
       |  fork ie | send ie ie ie | x, y <= recv. ie ie |  IT te
iE   ::=  iE lop ie | iv lop iE | setLabel iE | fork iE 
       |  send iE ie ie | send iv iE ie | send iv iv iE
       |  x, y <= recv. ie ie |  IT tE

tE   ::=  cdots | TI iE


x, y <= brecv . ie @= IT (let te' = TI (x, y <= recv. ie  (IT te')) in te')
\end{code}
...
{|
iconf iS (lconf id il tS (iniE (fork (IT te))), ldots)
-->
iconf iS' (lconf id il tS (iniE id'), ldots, lconf id' il tS te)
|}
{|fresh (id')|}
\and
\inferrule[I-send]
...
|il canFlowTo il'|\\
|iS(id') = Q|\\
|iS' = iS [ id' mapsto (il', id,  ie) cons Q ]|
}
{|
...
{|
iconf iS (lconf id il tS (iniE (x, y <= recv. ie1 ie2))), ldots)
-->
iconf iS' (lconf id il tS (iniE ({ie'/x,id'/y} ie1)), ldots)
|}
\and
\inferrule[I-noRecv]
{
