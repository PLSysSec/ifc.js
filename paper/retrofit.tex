...
%use a simple,
%untyped lambda calculus with mutable references and fixpoint in place of
%ECMAScript,
%to demonstrate some of the key properties without having too much
%detail
...
%The presentation is mostly standard, and utilizes
...
Figure~\ref{fig:embedding} provides all of the rules responsible for % actually
...
\Red{Ale: TODO we need to say that what is important about k is that there is no
sharing between tasks. Moreover, |klone| should be implementable either by the
target language or its runtime system}
% ---we will discuss   In Section~\ref{sec:concrete}, we give a more detailed
