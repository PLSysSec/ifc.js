...
which specify whether or not data tagged with one label |l1| can flow to another
...
a property called \emph{noninterference}.
...
labeling~\cite{Zeldovich:2006, lio} to its extreme: our starting point
...
are far more easily implementable and require minimal semantic changes.

Why is this setting more interesting than ``traditional
coarse-grained information flow control system on isolated
threads?''  First, although completely separate tasks
are semantically easy to reason about, real-world implementations often blur the
lines between tasks in the name of efficiency.  Characterizing what optimizations
are permissible is a problem we address in Section~\ref{sec:concrete}, and
it is quite subtle, since removing transitions from the operational semantics
of a language can break noninterference.  Second, even though the underlying
semantic model involves communicating tasks, these tasks can be coordinated
together in ways that simulate traditional features in monolithic languages.
In fact, it is a very useful design tool for discovering what variants of the
features are permissible and which are not.

A very simple example is a low security task sending a high security
...
For now, ignore |TI| and |IT|: roughly, this code creates a new
|sandbox|ed thread $i$ which waits (\textbf{blockingRecv}) for a
message, and then \textbf{send}s the thread a mutable reference to (|ref
true|).  If this operation actually shared the mutable cell, this cell
could be used to violate information flow control.  When working with
isolated tasks which each have their own heaps, the isolation is obviously true:
the sandboxed task receives a dangling pointer.

While this is a very simple example, we can still ask: given the
obviously safe formulation with separate heaps, can this fact be useful
for showing the safety of an semantics which only has a single heap
(Yes: see Section~\ref{sec:single-heap})?  Furthermore, the ``raw''
semantics violates expectations about how inter-thread communication
works on a shared heap: it strongly suggests, for example, that which
types of data may be passed between tasks should be restricted (for
example, in JavaScript, one might only allow JSON objects to be passed
between tasks).
...
these threads.  The recasting of the problem to one of scheduling
