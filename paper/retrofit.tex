...
\and
\inferrule[T-app]
{ } {| intE ((\x.te) tv) ~> intE ({tv/x} tv) |}

\and
\inferrule[T-ifTrue]
{ } {| intE (if true then te1 else te2)  ~> intE te1 |}

\and
\inferrule[T-ifFalse]
{ } {| intE (if false then te1 else te2) ~> intE te2 |}
...
|}
{|
iconf iS (lconf id il tS (IT (intE te)), ldots)
.->
iconf iS (lconf id il tS' (IT (intE te')), ldots)
|}
...
\inferrule[I-recv]
{
| filter il (iS(id)) = io1 :: cdots :: iok :: (il', id', ie')|\\
| iS' = iS[id mapsto io1 :: cdots :: iok]|\\
