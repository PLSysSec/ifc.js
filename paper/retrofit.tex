...
and for simplicity, we require the semantics to be deterministic.
This procedure will
...
some expressions |te|, some set of values |tv| and a deterministic
...
Now, messages can only be received by a task if its current label is
at least as high as the label of the message.


Specifically, receiving a message using
\[ |recvf ix1 ix2 ie1 ie2| \]
binds the senders task identifier and the message
...
\begin{itemize}
    ...
    primitives for asynchronously communicating between
    tasks and intermediating between the borders of the two languages.
\end{itemize}
