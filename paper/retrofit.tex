...
of a different evaluation context |curlyRedE|,
following the approach of Matthews and
Findler~\cite{Matthews:2007:OSM:1190216.1190220} to simplify combining
...
\caption{Simple untyped lambda calculus |targetLangML|}
\label{fig:ml}
\end{figure}
...
labels (|canFlowTo|, |lub| and |glb|),
    \item Operations for inspecting (\textbf{getLabel}) and modifying (\textbf{setLabel}) the current label of the task (since the current label's role is to protect everything in the task, the task can only increase its label),
    ...
    isolated tasks, described in more detail in the
    next section.
\end{itemize}

These operations are all defined with respect to an evaluation context
|curlyBlueE| represents the context of the current task which is running, which
has three important pieces of state: the global message queues |iS|, the
current label |il| and the task ID |id|.

At a high level, the semantics define a coarse-grained floating-label IFC
system~\cite{Zeldovich:2006, lio}, i.e., a system in which the current label of
the task can be raised to protect everything in scope;
%
this is different from traditional fine-grained language-based IFC systems that
associate labels with values~\cite{sabelfeld2003language,Austin:Flanagan:PLAS09}.
%
We note that while first class labels and operations for inspecting the current
label are are essentially universal to all floating-label systems, our choice
of communication and isolation primitives is motivated by those present in
browsers, namely \texttt{postMessage} and Workers~\cite{webworkers,webmessaging}.
%
Of course, other choices such as blocking communication or labeled channels are
possible and further discussed in Section~\ref{sec:extensions}.
%
Though motivated by Web Workers, our notion of a task generalizes previous
primitives considered by floating-label dynamic IFC
systems~\cite{Zeldovich:2006, lio, stefan:addressing-covert,
Hritcu:2013:YIB:2497621.2498098}.
...
non-interference claims will hold regardless of the choice of |klone|.
...
with |targetLangML|, we can choose to preserve booleans across
