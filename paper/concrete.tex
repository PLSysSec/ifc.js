...
\begin{definition}[Inclusion of information-flow control languages]
    A language \con{|(C, Ce, V, .->, erasef l)|} is included in another
    language \abs{|(C, Ce, V, .->, erasef l)|} if there exists a total function |f : tC -> iC| such that
...
\begin{definition}[Isomorphism of information-flow control languages]
    ...
    |tc'|$ then $f(|tc|)\ \ifc{\approx_l}\ f(|tc'|)$ and if $|ic|\ \ifc{\approx_l}\ |ic'|$ then $f^{-1}$) and
