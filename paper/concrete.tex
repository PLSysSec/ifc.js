...
show that the definition in Figure~\Red{XXX} satisfy these
conditions.
...
\begin{definition}[Non-Interference preserving, restricted isomorphism (NPRI)]
  ...
  |concreteLang alpha targetLang| (which can be thought of as
  the concrete language) a \textit{non-intereference preserving,
  restricted isomorphism} with regard to the IFC language
  |specLang alpha targetLang|
  (which can be thought of as the abstract language) if the following
  two conditions hold:
  \begin{enumerate}
...
\begin{theorem}[NPRI implies TSNI]
  If a language |concreteLang alpha targetLang| is a NPRI, then
  |concreteLang alpha targetLang| satisfies TSNI.
\end{theorem}


With this machinery in place, we can now state and prove that our
single-heap ML-like IFC language is NPRI, and thus non-interfering.
