...
motivation to preserve JavaScript's semantics \Red{and avoid any modifications
  to its engine}, while retrofitting it with dynamic information
flow control.

\Red{\sout{One approach to information flow control is fine-grained tracking,
...
\Red{Operating systems research (e.g.~\cite{Zeldovich:2006}) proposes
\textit{coarse-grained} approaches to IFC, where}
...
\Red{instead of at every instruction in the code~(e.g. \cite{JSFlow})}.
...
understanding and reasoning about \Red{the security guarantees} of the
system, without \sout{information flow} reasoning about \Red{most of} the
technical details of the semantics of the \Red{underlying} programming language.

In this paper, \Red{we present a framework which translates coarse-grained IFC
