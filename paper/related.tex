...
Designed to target legacy code, secure multi-execution (SME) is a black-box
technique to enforce information-flow control~\cite{Devriese:2010}. It runs
multiple copies of the program, one per security level, where the semantics of
I/O interactions is altered. Besides its black-box nature, authors often present
it considering one specific programming
language~\cite{KULeuven-350547,Rafnson:2013}. Differently, Bielova
...
interaction trees~\cite{jacobs-tutorial}, which threats programs as black-boxes
about which nothing is known except what is inferred from their I/O interactions
with the environment. Similar to SME, our approach only mediates some I/O
operations; differently, it runs the program once.
...
scenario~\cite{Yip:2009:PBS,DeGroef:2012,conf/esorics/AkhaweLHSS13}. Different
