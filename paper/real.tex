...
and hardware.
%
This isolation can be achieved using hardware protection
mechanisms~\cite{Belay:2012:DSU:2387880.2387913} (similar to a simpler
version of Wedge~\cite{Bittau:2008:WSA:1387589.1387611} with an
...
and it associates labels with threads (tasks).
%
...
implementation is that it allows use of Haskell's efficient implementation of
threads, channels, etc. (e.g., we use Haskell's \texttt{forkIO} to
fork a lightweight thread in the case of |fork|), in contrast to
defining them in a completely pure fashion (as suggested in Section~\ref{sec:monad}).

What is the interpretation of this system as per Section~\ref{sec:retrofit}?
%
Here, the \emph{pure subset} of Haskell is the target language
|targetLangHS|, while the monadic subset of Haskell in the \verb|LIO|
...
by running multiple instances of the JS runtime, i.e., |targetLangJS|, in
...
From here, the IFC language functionality is implemented in the JS
runtime, much like browser layout engines implement the DOM, and
expose the new functions, e.g., |sandbox|, by attaching them to the JS
global object.
%
To avoid termination timing attacks, each task created with |sandbox| is
executed in a separate thread, running a separate, \emph{fresh} instance of our
modified JS runtime.
%
Formally, \verb|sandbox| is defined with |klone(tS) =
