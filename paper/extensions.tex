...
%applications.
%%
In fact, COWL provides a labeled version of the XHR object to perform external
side-effects as well as labeled messages to avoid raising the
floating-label in certain situations.
...
In this section, we show how to extend our framework with several such
...
values similarly as~\cite{lio, Hritcu:2013:YIB:2497621.2498098}:
%, much like those of LIO and
|iv ::= cdots inlinesep Labeled il ie|.
%
%Following LIO,
We say that the expression |ie| is protected by label |il|,
while the label |il| itself is protected by the task's current label.
%
%
The label of such values can be inspected the a given task without
...
\label{sec:clearance}
%
Similar to other systems, COWL additionally provide a discretionary access
control (DAC) mechanism---called \emph{clearance}---at the language
level~\cite{Zeldovich:2006,Hritcu:2013:YIB:2497621.2498098, lio}.
%
...
%
To extend our framework with clearance, it is enough to
a family of predicates to each rule
...
reads above the clearance).  More specifically, the predicate $P := |il
...
%parties.
In COWL, as well as other decentralized IFC systems~\cite{myers:dlm}, a
computation is executed with a set of \emph{privileges}, which, when exercised,
allow the computation to declassify data (e.g., by lowering the current label).
%
...
%straight forward.
%
The challenge with such an extension lies in the precise security guarantees
that must be proved, which, to the best of our knowledge, is an open research
problem.
%
%COWL provides support for privileges,
...
In this section, we state some limitations of our formal approach, and
outline potential future directions to alleviate them. %these limitations.

\subsection{Type Safety}
\label{sec:extensions:types}
\Red{Ale: I don't get this part. Why are we talking about type-safety? Is the
  problem that the combined semantics does not get stuck when the target does?
  Is type-safety a manner to get progress?}
...
%
For simplicity, we do not treat more-general class of schedulers;
...
where programs are secure only if |l|-equivalent results are
deterministic.
...
%
For example, COWL must load resources or perform XHR to be useful.

Like labeled references, features with external effects must be explicitly
...
%% %
Alternatively, COWL and JSFlow wrap unsafe primitives
%is, for example, used in COWL and JSFlow, respectively,
in order to allow for controlled network communication.
%
