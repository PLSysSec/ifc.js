...
% languages.
%This requires an appropriate definition of this notion
%that is language agnostic, so

In this section, we present a few general and target-language agnostic
definitions for what an information flow control language is and what
non-interference properties it may have.  We also state theorems about our
system. %which we prove in Section~\ref{sec:proofs}.  In particular, we show
that |specLang alpha targetLang|, with an appropriate scheduler |alpha|,
satisfies non-interference~\cite{Goguen82}, without making any reference to
properties of |targetLang|. For technical details, interested readers can refer
to the extended version of this paper~\footnote{\url{http://informationflowcontrol.net/ifc-inside/paper.pdf}}.
...
TSNI:
%
consider a target language with a standard branching construct |if e then e else
e|
%similar to |targetLangML|
and an additional expression terminal |diverge| that denotes a divergent
computation, i.e., |diverge| always reduces to |diverge| and a simple label
lattice |{pub, sec}| such that |canFlowTo2 pub sec|, but |cantFlowTo2 sec pub|.
