...
tasks or messages.
%
While such assumptions are standard~\tocite{}, our defintions allow for
stronger attackers that may be able to inspect resource usage.\footnote{
  We belive that we can extend |specLang alpha (targetLang nop)| to
  such models using the resource limits techniqes of~\tocite{}.
  %
  We leave this extension to future work.
}

Non-inteference relies on the defintion of an attacker's observational power at
...
distinguish two configurations that are |l|-equivlaent.
%
Since our erasure function captures attacker mode, we simply define this
equivalance as the syntactic-equivalence of erased configurations.
%
